...
void CoordinatesTransition::pz90_to_sk42() {
    pz90_B = get_pz90_latitude(); //перевод широты для удобства переноса формулы (B)
    pz90_L = get_pz90_longitude(); //перевод долготы для удобства (L)
    pz90_zone_n = static_cast<int>((6 + pz90_L) / 6); //номер шестиградусной зоны (n)
    pz90_distance_l = (pz90_L - (3 + 6 * (pz90_zone_n - 1))) / 57.29577951; //расстояние от определёной точки до осевого меридиана зоны(l)
    pz90_answ_X = 6367558.4968 * pz90_B - sin (2 * pz90_B) * (16002.8900 + 66.9607 * sin_pz90_B(2) + 0.3515 * sin_pz90_B(4) -
    ...
    pz90_answ_Y = (5 + 10 * pz90_zone_n) * pow(10, 5) + pz90_distance_l * cos(pz90_B) * (6378245 + 21346.1415 * sin_pz90_B(2) + 107.1590 * sin_pz90_B(4)
